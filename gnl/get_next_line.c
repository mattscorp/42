...
/*   Updated: 2018/12/10 10:53:23 by mascorpi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char			*ft_read_tot(char **tot, int fd)
{
	...
	while ((nb_read = read(fd, read_tmp, BUFF_SIZE)) > 0)
	{
		tot_tmp = tot[fd];
		read_tmp[nb_read] = '\0';
		if (!(tot[fd] = ft_strjoin(tot[fd], read_tmp)))
		{
			ft_strdel(&tot[fd]);
			return (NULL);
		}
...
int					get_next_line(const int fd, char **line)
{
	...
	tot_tmp = tot[fd];
	if (!(tot[fd] = ft_strsub(tot[fd], ft_cl(tot[fd]), ft_strlen(tot[fd]))))
	{
		ft_strdel(&tot[fd]);
		return (-1);
	}
